...
public class ServiceDescriptionUpdater {
    ...
    discoveryClient.getServices().stream().forEach(serviceId -> {
      ...
      if (serviceInstances == null || serviceInstances.isEmpty()) {
        ...
        if (jsonData.isPresent()) {
          definitionContext.addServiceDefinition(serviceId, getJSON(jsonData.get()));
        } else {
          logger.error(
  ...
  private Optional<Object> getSwaggerDefinitionForAPI(String serviceName, String url) {
    logger.debug("Accessing the SwaggerDefinition JSON for Service : {} : URL : {} ", serviceName,
        url);
    try {
      Object jsonData = template.getForObject(url, Object.class);
      return Optional.of(jsonData);
    } catch (RestClientException ex) {
      logger.error("Error while getting service definition for service : {} Error : {} ",
